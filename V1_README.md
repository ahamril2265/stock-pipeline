...
*   **Register trade schema:**
    ```bash
    python3 register_trade_schema.py
    ```
*   **Register price tick schema:**
    ```bash
    python3 register_price_tick_schema.py
    ```

### 6️⃣ Start Unified Producer
```bash
python3 unified_avro_producer.py
...
#If spark module not found:
  use 'docker exec -it spark-master sh' and 'python3 -m pip install <module>'