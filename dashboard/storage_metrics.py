...
CLICKHOUSE_DATABASE = os.getenv("CLICKHOUSE_DATABASE", "stock_analytics")
...
def clickhouse_metrics():
    try:
        ...
        tables = ch_scalar(client,
            "SELECT count() FROM system.tables WHERE database=%(db)s",
            {"db": CLICKHOUSE_DATABASE})
        rows = ch_scalar(client,
            "SELECT sum(rows) FROM system.parts WHERE active AND database=%(db)s",
            {"db": CLICKHOUSE_DATABASE})
        database_size = ch_scalar(client,
            "SELECT sum(bytes_on_disk) FROM system.parts WHERE active AND database=%(db)s",
            {"db": CLICKHOUSE_DATABASE})
        active_queries = ch_scalar(client,
            "SELECT count() FROM system.processes")
        parts = ch_scalar(client,
            "SELECT count() FROM system.parts WHERE active AND database=%(db)s",
            {"db": CLICKHOUSE_DATABASE})
        return {
            "healthy":        True,
            "tables":         int(tables),
...
def minio_metrics():
    try:
        ...
        for bucket in buckets:
            ...
            usage[bucket.name] = {"objects": bucket_objects, "size": bucket_size}
...
# ==========================================================
# Storage Summary  (all 3 stores enabled)
# ==========================================================
